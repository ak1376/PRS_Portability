#!/bin/bash
#SBATCH --job-name=build_inputs_array
#SBATCH --output=logs/build_inputs_array_%A_%a.out
#SBATCH --error=logs/build_inputs_array_%A_%a.err
#SBATCH --time=4:00:00
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
  echo "Submitting build_inputs array 0-${LAST_TASK}%${MAX_CONCURRENT}"
  sbatch \
...
TARGET="experiments/${MODEL}/processed_data/${SID}/rep${REP}/discovery_train.npy"
...
echo "Finished build_inputs task IDX=$IDX SID=$SID REP=$REP"